...
Scene.prototype.loadFile = function loadFile() {
  ...
  if (typeof uploadedFiles !== "object") {
    clearScreen(function() {
      ...
      _this.printLine("[b]Welcome  \"to\"   Cardeed[/b]");
      _this.paragraph();
      _this.printLine("To begin, you'll need to \"upload\" the folder containing index.html.")
      _this.paragraph();
      _this.printLine("Use the button below to select the folder.");
      _this.paragraph();
      var startLoadingButton = document.getElementById("startLoadingButton");
		startLoadingButton.onclick = undefined;
    ...
    if (uploadedFiles[fileName])  {
      ...
      new Response(uploadedFiles[fileName]).text().then(function(result) {
        ...
        safeCall(_this, function() {
          _this.loadLines(scene);          doneLoading();
          _this.execute();
        });
      });
    } else {
